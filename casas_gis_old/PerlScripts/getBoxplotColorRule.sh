...
# Color list.
# GMT_panoply.cpt
rule="255:255:204-217:240:163-217:240:163-173:221:142-173:221:142-120:198:121-120:198:121-65:171:93-65:171:93-35:132:67-35:132:67-0:90:50"
# https://www.colorspire.com/rgb-color-wheel/
outlierLow="255:255:204"
outlierHigh="0:90:50"
...
rule_file_header="# Viridis modified from http://soliton.vm.bytemark.co.uk/pub/cpt-city/mpl/viridis.cpt\n# to raster $rasterMapName\n"
...
cd "$HomeDir"
output="${rasterMapName}_boxplot_color_rule.txt"
echo "$output"

# Put colors into an array.
IFS='-' read -r -a colors <<< "$rule"
if [[ "$reverseColors" == "yes" ]]; then
    colors=("${colors[@]}")
fi
...
if [[ "$divergentRule" == "divNo" ]]; then
    ...
    {
        echo "$rule_file_header"
        if [[ "$percent" == "yes" ]]; then
            # Outlier low
            if (( $(echo "$absMin != $min" | bc -l) )); then
                echo "$(printf "%.3f" "$absMin")% $outlierLow"
                echo "$(printf "%.3f" "$min")% $outlierLow"
            fi
            # Non outlier part
            for ((k = 0; k < numOfColors; k++)); do
                echo "$(printf "%.3f" "${coefficients[$k]}")% ${colors[$k]}"
            done
            # Outlier high
            if (( $(echo "$absMax != $max" | bc -l) )); then
                echo "$(printf "%.3f" "$max")% $outlierHigh"
                echo "$(printf "%.3f" "$absMax")% $outlierHigh"
            fi
        else
            # Outlier low
            if (( $(echo "$absMin != $min" | bc -l) )); then
    ...
    if (( $(echo "$min < 0" | bc -l) )); then
        if (( $(echo "$scaleCenter == 0" | bc -l) )); then
            # Build array of coefficients.
            baseCoeff=$(echo "scale=2; 1 / $halfNumOfBands" | bc)
            coefficients=("$min")
            for ((i = halfNumOfBands - 1; i >= 1; i--)); do
                coefficients+=($(echo "$i * $baseCoeff * ($scaleCenter + $min)" | bc))
            done
            if (( numOfColors % 2 == 1 )); then
                coefficients+=("$scaleCenter")
            else
                coefficients+=("$scaleCenter" "$scaleCenter")
            fi
            for ((i = 1; i <= halfNumOfBands - 1; i++)); do
                coefficients+=($(echo "$i * $baseCoeff * ($max - $scaleCenter)" | bc))
            done
            coefficients+=("$max")
        elif (( $(echo "$scaleCenter != 0" | bc -l) )); then
            # Build array of coefficients.
            baseCoeff=$(echo "scale=2; 1 / $halfNumOfBands" | bc)
            coefficients=("$min")
            for ((i = halfNumOfBands - 1; i >= 1; i--)); do
                coefficients+=($(echo "$i * $baseCoeff * ($min - $scaleCenter) + $scaleCenter" | bc))
            done
            if (( numOfColors % 2 == 1 )); then
                coefficients+=("$scaleCenter")
            else
            ...
            for ((i = 1; i <= halfNumOfBands - 1; i++)); do
                coefficients+=($(echo "$i * $baseCoeff * ($max - $scaleCenter) + $scaleCenter" | bc))
            done
            coefficients+=("$max")
        fi
    else
        ...
        for ((i = 1; i <= halfNumOfBands - 1; i++)); do
            coefficients+=($(echo "$scaleCenter + ($i * $baseCoeff * ($max - $scaleCenter))" | bc))
        done
        coefficients+=("$max")
    fi
