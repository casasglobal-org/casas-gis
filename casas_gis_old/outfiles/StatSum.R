...
# Function for  coefficient of quartile variation.
# http://en.wikipedia.org/wiki/Quartile_coefficient_of_dispersion
coeff.quart.disp <- function(x, na.rm = FALSE) {
	((quantile(x, na.rm = na.rm, names = FALSE)[4] - 
    quantile(x, na.rm = na.rm, names = FALSE)[2]) / 
    (quantile(x, na.rm = na.rm, names = FALSE)[4] + 
    quantile(x, na.rm = na.rm, names = FALSE)[2]))
}

########################### Compute Avg, SD, CV, CQD, IQR ####################
...
for ( i in seq(along = varNames) ) {
	...
	if ( varNames[i] == "Bloomday" ) {
		...
		remove(y)	        
		# Compute coefficient of quartile variation.
        # http://en.wikipedia.org/wiki/Quartile_coefficient_of_dispersion
		y <- aggregate(allData[[varNames[i]]],
						by = list(WxFile = allData$WxFile), 
						FUN = coeff.quart.disp, na.rm = TRUE)
		names(y)[2] <- paste(varNames[i])
		# Replace NaN with zeros.
		y[is.na(y)] <- 0.0
		cqd.data <- merge(cqd.data, y, by = "WxFile")
		remove(y)
		...
		# Replace NaN with zeros.
		y[is.na(y)] <- 0.0
		iqr.data <- merge(iqr.data, y, by = "WxFile")
		remove(y)		
	...
	##### BloomYr #####		
        ...
        cqd.data <- merge(cqd.data, y, by = "WxFile") # No change in CQD
        iqr.data <- merge(iqr.data, y, by = "WxFile") # No change in IQR
		remove(y)
...
cqd.data <- subset(cqd.data, select = names(allData))
iqr.data <- subset(iqr.data, select = names(allData))

# Write output files.
baseFileName <- substr(fileNames[1], 1, (nchar(fileNames[1]) - 9))
...
write.table(cv.data, file = paste(baseFileName, "CV.txt", sep = ""), 
			quote=FALSE,  sep="\t", 
			row.names = FALSE, col.names = TRUE)
write.table(cqd.data, file = paste(baseFileName, "CQD.txt", sep = ""), 
			quote=FALSE,  sep="\t", 
			row.names = FALSE, col.names = TRUE)
write.table(iqr.data, file = paste(baseFileName, "IQR.txt", sep = ""), 
			quote=FALSE,  sep="\t", 
			row.names = FALSE, col.names = TRUE)
