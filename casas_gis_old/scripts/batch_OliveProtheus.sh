...
for i in 12 13 14 16 17 18 20 22 23 24 25 28 30; do
    ...
    if [ $i -eq 28 ]; then
        ...
        # Run GIS routine
        medPresentClimate -c -p -m SaveDir="$directory" longitude=5 latitude=6 year=11 parameter="$parameter" interpolation="idw" lowercut=0 uppercut=0 region=-1 alt=900 resolution=1 legend1="$legend"
    fi
