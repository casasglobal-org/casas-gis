...
run_plant_only() {
    ...
    for i in 12 13 14 16 17 18 19 20 23; do
        ...
        if [ $i -eq 12 ]; then
            ...
            # Run GIS routine
            map.pbdm.andalusia -r -p save_directory="$directory" \
                longitude=5 latitude=6 year=11 parameter="$parameter" \
                interpolation=idw numpoints=3 legend1="$legend" \
                color_rule_regular="${turbo_color_rule}" lowercut=0 uppercut=0 provinces=all crop=sigpac_buffered_3km cropthreshold=0.20 alt=1000 resolution=1
            wait
        fi
